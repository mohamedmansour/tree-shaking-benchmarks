...
class RolldownBuildAction extends BaseAction {
  ...
  public async build(entryPoints: Record<string, string>, name: string): Promise<StatResult> {
    ...
    const outputDir = path.join(DIST_DIR, 'rolldown', name)
    ...
    try {
      ...
      // Copy static files
      copyFolder(`webuis/${name}`, `dist/rolldown/${name}`, ['.ts', '.tsx', '.d.ts', '.js'])
...
export default function (options: ActionOptions): Promise<Array<StatResult>> {
  const action = new RolldownBuildAction(options)
  return action.run()
}
